...
const Chat = (props: any) => {
    ...
    useEffect(() => {
        const fetching = async () => {
            const response = await fetch('https://tealcian-backend-production-3d2b.up.railway.app/chat/getMessages', {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Access-Control-Allow-Origin": "<origin>"
                },
                body: JSON.stringify({roomId: props.data.privateId, refreshToken: localStorage.getItem('refreshToken')})
            })
    ...
    useEffect(() => {
      if(massages.length > 0){
        ...
        console.log(dataChat[dataChat.length - 1].body)
      }
    })
        ...
        return (
            <div className=" flex flex-col  h-screen" >
                <div className=" flex mt-[24px] mb-[18px] ml-6">
                    <button className="hidden third hover:bg-gray-300 rounded-lg cursor-pointer w-[40px] h-[40px] mt-auto mb-auto mr-5"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" data-slot="icon" className=" w-[30px] h-[30px] mr-auto ml-auto">
                ...
                <div className=" flex justify-center items-center p-2 w-full ">
                    <input type="text" value={inputValue} onKeyDown={handleKeyBoard} id="typing" onChange={onChange} className="bg-gray-50 border w-[250px] mr-3 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Type...." required />
